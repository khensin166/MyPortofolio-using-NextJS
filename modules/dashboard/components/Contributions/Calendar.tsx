          ...
          {weeks?.map((week) => (
            <div key={week.firstDay}>
              {week.contributionDays.map((contribution) => {
                ...
                const levelClass =
                  colorIndex !== -1 ? contributionLevels[colorIndex!] : null;

                const backgroundColorClass =
                  contribution.contributionCount > 0 ? levelClass : "bg-muted";
                ...
                return (
                  <motion.span
                    ...
                    className={clsx(
                      "my-[2px] block h-[12px] w-[12px] rounded-sm transition-colors duration-200",
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-sm">
          ...
          <ul className="flex gap-1">
            ...
            {contributionLevels.map((item, index) => (
              <motion.li
                key={item}
                initial="initial"
                animate="animate"
                ...
                className={clsx("h-[10px] w-[10px] rounded-sm", item)}
              />
            ))}
          </ul>
        ...
        <div
          className={clsx(
            ...
            "rounded bg-secondary px-2 text-sm",
          )}
        >
