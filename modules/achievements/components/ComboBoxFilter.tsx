...
const ComboBoxFilter = ({
  ...
  return (
    <div ref={comboBoxRef} className="relative w-full md:w-[230px]">
      <Button
        className="flex w-full items-center justify-between gap-4 bg-secondary p-2 text-foreground outline outline-border hover:bg-muted transition-all duration-300"
        onClick={handleClickOpen}
        data-posthog-event="click_filter_achievements"
      >
          ...
          >
            <div className="w-full rounded-md bg-card outline outline-border shadow-xl">
              <div className="grid w-full grid-cols-[1.5rem_1fr] items-center border-b border-border px-3 py-2">
                ...
                <input
                  type="search"
                  className="flex justify-start bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground"
              ...
              <div className="p-1">
                ...
                {filteredData?.map((item, index) => (
                  <button
                    key={index}
                    className="grid w-full grid-cols-[1.5rem_1fr] items-center rounded-[4px] p-2 text-foreground hover:bg-muted transition-colors duration-200"
